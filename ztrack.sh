...
print_banner() {
    ...
    echo -e "${NC}"
    echo -e "${BLUE}  ═══════════════════════════════════════════════════════════════════════════════${NC}"
    echo -e "${WHITE}                    ZTrack - OSINT Digital Footprint Finder v1.0${NC}"
    echo -e "${BLUE}  ═══════════════════════════════════════════════════════════════════════════════${NC}"
    echo ""
}
...
print_menu() {
    ...
    echo -e "${BLUE}╠═══════════════════════════════════════════════════════════════╣${NC}"
    echo -e "${BLUE}║${NC}  ${RED}[1]${NC} 🔍 Username Search     - Find across 100+ platforms     ${BLUE}║${NC}"
    echo -e "${BLUE}║${NC}  ${RED}[2]${NC} 🛡️  Email Breach Check   - Check HaveIBeenPwned          ${BLUE}║${NC}"
    echo -e "${BLUE}║${NC}  ${RED}[3]${NC} 🔐 Password Breach      - Check if password is leaked   ${BLUE}║${NC}"
    echo -e "${BLUE}║${NC}  ${RED}[4]${NC} 🌐 Google Dork Search   - Advanced Google search        ${BLUE}║${NC}"
    echo -e "${BLUE}║${NC}  ${RED}[5]${NC} 🗑️  Removal Tools        - Data broker opt-outs         ${BLUE}║${NC}"
    echo -e "${BLUE}║${NC}  ${RED}[6]${NC} 📊 View Last Results    - Show previous scan            ${BLUE}║${NC}"
...
check_site() {
    ...
    if [[ "$response" == "200" ]]; then
        echo -e "${GREEN}[+]${NC} ${WHITE}$site_name${NC}: ${BLUE}$url${NC}"
...
search_username() {
    ...
    echo ""
    echo -e "${RED}═══════════════════════════════════════════════════════════════${NC}"
    echo -e "${WHITE}  Scanning for username: ${RED}@$username${NC}"
    echo -e "${RED}═══════════════════════════════════════════════════════════════${NC}"
    echo ""
    
    # Create results file
    RESULTS_FILE="$OUTPUT_DIR/scan_${username}_$(date +%Y%m%d_%H%M%S).txt"
    echo "# ZTrack Scan Results" > "$RESULTS_FILE"
    ...
    if command -v parallel &> /dev/null; then
        ...
        export -f check_site
        export TIMEOUT RESULTS_FILE GREEN BLUE WHITE GRAY RED NC
        ...
        for site_name in "${!SITES[@]}"; do
            ...
            if [[ "$response" == "200" ]]; then
                echo -e "\r${GREEN}[+]${NC} ${WHITE}$site_name${NC}: ${BLUE}$url${NC}          "
                echo "$site_name|$url|FOUND" >> "$RESULTS_FILE"
                ((found++))
            fi
        done
...
check_email_breach() {
    ...
    echo ""
    echo -e "${BLUE}[*] Checking Hudson Rock (Infostealer DB)...${NC}"
...
google_dork_search() {
    ...
    echo ""
    echo -e "${BLUE}═══════════════════════════════════════════════════════════════${NC}"
    echo -e "${WHITE}  Google Dork Queries for: ${RED}@$username${NC}"
    echo -e "${BLUE}═══════════════════════════════════════════════════════════════${NC}"
    echo ""
    
    echo -e "${WHITE}Copy and paste these into Google:${NC}"
    echo ""
    echo -e "${BLUE}General Search:${NC}"
...
view_last_results() {
    ...
    grep "FOUND" "$latest" | while IFS='|' read -r site url status; do
        echo -e "${GREEN}[+]${NC} ${WHITE}$site${NC}: ${BLUE}$url${NC}"
    done
...
main() {
    ...
    case "$1" in
        ...
        --generate-gdpr)
            ...
            ;;
        -h|--help)
            echo "ZTrack - OSINT Digital Footprint Finder"
            echo ""
    ...
    while true; do
        print_menu
        read -p "$(echo -e ${BLUE}Enter choice [0-6]: ${NC})" choice
